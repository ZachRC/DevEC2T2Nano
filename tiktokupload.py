...
async def random_action(page):
    actions = [
        ...
        lambda: page.evaluate(f"window.scrollBy(0, {random.randint(300, 1000)})"),
...
async def interact_with_video(page):
    try:
        video = await page.wait_for_selector("div[data-e2e='recommend-list-item-container']", timeout=5000)
        await video.click()
